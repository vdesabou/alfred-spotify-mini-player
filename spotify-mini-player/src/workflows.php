...
class Workflows
{
    ...
    public function tojson($a = null)
    {
        if (is_null($a) && !empty($this->results)):
            $a = $this->results; elseif (is_null($a) && empty($this->results)):
            return false;
        endif;
...
//EOB
            ...
            foreach ($c_keys as $key):                        // For each of those keys
                if ($key == 'uid'):
                    if ($b[$key] === null || $b[$key] === ''):
                        continue; else:
                        $c->uid = $b[$key];
        endif; elseif ($key == 'arg'):
                    $c->arg = $b[$key]; elseif ($key == 'type'):
                    $c->type = $b[$key]; elseif ($key == 'quicklookurl'):
                    $c->quicklookurl = $b[$key]; elseif ($key == 'valid'):
                    if ($b[$key] == 'yes' || $b[$key] == 'no'):
                       $c->valid = $b[$key];
        endif; elseif ($key == 'autocomplete'):
                    if ($b[$key] === null || $b[$key] === ''):
                        continue; else:
                        $c->autocomplete = $b[$key];
        endif; elseif ($key == 'icon'):
                    $icon = new StdClass();
        if (substr($b[$key], 0, 9) == 'fileicon:'):
                        $val = substr($b[$key], 9);
        $icon->path = $val;
        $icon->type = 'fileicon'; elseif (substr($b[$key], 0, 9) == 'filetype:'):
                        $val = substr($b[$key], 9);
        $icon->path = $val;
        $icon->type = 'filetype'; else:
                        $icon->path = $b[$key];
        endif;
        $c->icon = $icon; elseif ($key == 'subtitle'):
        ...
        $c->mods = $mods; else:
                        $c->$key = $b[$key];
        endif; elseif ($key == 'text' && gettype($b[$key]) == 'array'):
                    $text = new StdClass();
    ...
    public function toxml($a = null, $format = 'array')
    {
        ...
        $c->$key = $b[$key]; elseif ($key == 'type'):
                    $c->addAttribute('type', $b[$key]); elseif ($key == 'valid'):
                    if ($b[$key] == 'yes' || $b[$key] == 'no'):
                    ...
                    if (substr($b[$key], 0, 9) == 'fileicon:'):
                        $val = substr($b[$key], 9);
        $c->$key = $val;
        $c->$key->addAttribute('type', 'fileicon'); elseif (substr($b[$key], 0, 9) == 'filetype:'):
                        $val = substr($b[$key], 9);
        $c->$key = $val;
        $c->$key->addAttribute('type', 'filetype'); else:
                        $c->$key = $b[$key];
        endif; elseif ($key == 'subtitle'):
                    if (gettype($b[$key]) == 'array'):
                        $subtitle_types = array('shift', 'fn', 'ctrl', 'alt', 'cmd');
        $subtitles = $b[$key];
        $subtitle_keys = array_keys($subtitles);
        ...
        if (is_array($a)):
            if (file_exists($b)):
                if (file_exists($this->path.'/'.$b)):
                    $b = $this->path.'/'.$b;
        endif; elseif (file_exists($this->data.'/'.$b)):
                $b = $this->data.'/'.$b; elseif (file_exists($this->cache.'/'.$b)):
                $b = $this->cache.'/'.$b; else:
                $b = $this->data.'/'.$b;
        endif; else:
            if (file_exists($c)):
                if (file_exists($this->path.'/'.$c)):
                    $c = $this->path.'/'.$c;
        endif; elseif (file_exists($this->data.'/'.$c)):
                $c = $this->data.'/'.$c; elseif (file_exists($this->cache.'/'.$c)):
                $c = $this->cache.'/'.$c; else:
                $c = $this->data.'/'.$c;
        endif;
        endif;
    ...
    public function write($a, $b)
    {
        ...
        return true; elseif (is_string($a)):
            file_put_contents($b, $a);
    ...
    public function read($a, $array = false)
    {
        if (file_exists($a)):
            if (file_exists($this->path.'/'.$a)):
                $a = $this->path.'/'.$a;
        endif; elseif (file_exists($this->data.'/'.$a)):
            $a = $this->data.'/'.$a; elseif (file_exists($this->cache.'/'.$a)):
            $a = $this->cache.'/'.$a; else:
            return false;
        endif;
        ...
        if (!is_null(json_decode($out)) && !$array):
            $out = json_decode($out); elseif (!is_null(json_decode($out)) && !$array):
